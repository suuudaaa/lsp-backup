<template>
  <div class="container">
    <div class="stock-brand-detail container">
      <div class="ld-brand-detail">
        <bread-crumb></bread-crumb>
        <h3 class="brand-name">{{params.goodsBrand}}</h3>
        <div class="stock-brand-group1">
          <el-row :gutter="12">
            <el-col :span="12">
              <div class="s-b-group1-left">
                <el-card style="height:325px;">
                  <div>
                    ...
                    <ul class="in-order-des">
                      <li v-for="(item,index) in inOrderTotalNum1" :key="index">
            ...
            <el-col :span="12">
              <div class="in-order-title-title1">
                <el-row>
                  <el-col :span="18">
                    <span class="txt-weight">出库货物规格</span>
                  </el-col>
                  <el-col :span="6">
                    <p class="recent-ser">
              ...
              <specs-type-table :tab-index="1" :t-order-time="brandsTime"></specs-type-table>
            </el-col>
          </el-row>
        </div>
      ...
      <div class="stock-line-chart" v-if="isLineChart">
        ...
        <div class="ld-line-chart">
          ...
          <lineChartExample :title="'该品牌库内货物数量走势'" :width="800" :data="lineChartData"></lineChartExample>
        </div>
      </div>
    </div>
  </div>
...
import {
  getGoodsBrandDetailTotal,
  getGoodsBrandDetailTrend
} from "@/api/getData";
...
export default {
  ...
  mounted() {
    ...
    let obj = {
      ...
      goodsBrand: this.$route.query.goodsBrand
    };
    this.params = obj;
  ...
  methods: {
    getBrandDetailData() {
      let that = this;
      getGoodsBrandDetailTotal(that.params).then(res => {
        if (res.ret == 0) {
          let data = res.data;
          ...
          for (let i = 0; i < data.length; i++) {
            if (data[i].name == "在库数量" || data[i].name == "可用数量") {
              let num = toThousands(data[i].num);
              arr.push({ name: data[i].name, num: num });
              that.inOrderTotalNum = arr;
            } else {
              arr1.push(data[i]);
              that.inOrderTotalNum1 = arr1;
            }
          }
        } else {
    ...
    getLineChartData() {
      ...
      getGoodsBrandDetailTrend(that.params).then(res => {
        if (res.ret == 0) {
          date = res.data.date;
