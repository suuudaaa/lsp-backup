...
export default {
  ...
  mounted() {
    ...
    this.stockAgeFun(); //库年龄
    this.stockWeekFun(); //库周
    this.mapFun();

    this.initTrendChart1(); //下个版本再做
  },
  methods: {
    getGoodsQtyFun() {
      getGoodsQty()
    ...
    orderStateFun() {
      orderState()
        .then(res => {
          if (res.ret == "0") {
            let data = JSON.parse(res.data);
            // console.log("orderState data begin:", data);
            let total = parseInt(data[0].number) + parseInt(data[1].number); //todo parseInt
            // let result = {
            //   fjd: this.GetPercent(data[0].number, total),
            //   jd: this.GetPercent(data[1].number, total)
            // };
            let result = [
              {
                value: data[0].number,
    ...
    initLineMap() {
      ...
      console.log("gzgzgzgzgzgzg:", [["广州", GZData]]);
